...
- An accessibility service **is** used, and only for the one measurement that has no other
  unprivileged route. It is off at two independent gates, cannot read screen content, and cannot
  see keystrokes — see the section above. Up to v0.2.0 this line read *"no accessibility service
  is used as a workaround"*; that was true then and is no longer, so it is corrected here rather
